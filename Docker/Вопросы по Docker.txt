...
- docker compose upзапускает контейнерную среду.
- docker compose psпредоставляет информацию о запущенных контейнерах и перенаправлении портов.
- docker compose logsполучает доступ к журналам для вашего контейнера.
- docker compose pauseприостанавливает контейнер и docker compose unpauseвозобновляет его.
- docker compose stopостановит контейнер.
- docker compose downудаляет контейнеры, сети и тома, связанные со средой.
