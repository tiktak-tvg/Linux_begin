# P.s. При просмоте моего резюме было высказывание, что у меня мало опыта, не имея со мной собеседования.) Хотелось бы спросить таких чудиков, вы какой опыт хотите? Работа в ИТ сфере более 20 лет это уже опыт. 
# Linux_begin.<br>
...
``locate hello``<br>
``cat (concatenation, firstly, In pair with split command)`` сегодня используется для вывода на экран содержимого файла<br>
...
##### man re_format
Общая схема регулярного выражения регулярное выражение состоит из трёх основных частей:<br>
```
...
> Пример: работает юзер через терминал. Смотрим сеансы <br>
``pts (ps ax | grep userX)`` <br>
